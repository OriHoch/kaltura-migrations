...
class Kaltura_Client_LiveChannelService extends Kaltura_Client_ServiceBase
{
	...
	function appendRecording($entryId, $mediaServerIndex, Kaltura_Client_Type_ServerFileResource $resource, $duration)
	{
		$kparams = array();
		...
		$this->client->queueServiceActionCall("livechannel", "appendRecording", null, $kparams);
		if ($this->client->isMultiRequest())
			return $this->client->getMultiRequestResult();
		$resultXml = $this->client->doQueue();
		$resultXmlObject = new \SimpleXMLElement($resultXml);
